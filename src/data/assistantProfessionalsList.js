...
const assistantProfessionalsList = staff
  .filter(
    s => (s.currentRank && s.currentRank.toLowerCase().includes('assistant professional'))
  )
  .map((s, idx) => ({
    id: idx + 1,
    staffId: s.id,
...
export default assistantProfessionalsList;
