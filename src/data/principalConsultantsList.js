...
// Auto-generate principal consultants list from staff records
const principalConsultantsList = staff
  .filter(
    s => (s.currentRank && s.currentRank.toLowerCase().includes('principal consultant'))
  )
  .map((s, idx) => ({
    id: idx + 1,
    staffId: s.id,
...
export default principalConsultantsList;
